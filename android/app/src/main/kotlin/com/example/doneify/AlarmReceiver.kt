...
class AlarmReceiver : BroadcastReceiver() {
    ...
    private fun buildNotification(
        ...
        b.setSmallIcon(R.drawable.doneify_notification_icon)
            ...
            .addAction(
                android.R.drawable.ic_lock_idle_alarm, "Snooze",
                pendingAlarmIntent
            )
            .setFullScreenIntent(
                buildPendingIntent(
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        Thread {
            ...
            if (repeatStatus == "once" || (repeatStatus != "once" && LocalDate.now() <= LocalDate.parse(
                    repeatEnd,
                    DateTimeFormatter.ofPattern("dd/MM/yyyy", Locale.ENGLISH)
                ))
            ) {
                ...
                if (intent.action == "snooze") {
                    ...
                    Log.d(
                        "debugging",
                        "received alarm, alarmId: $alarmId, taskId: $taskId, taskName: $taskName"
                    )
                    if (!finished!!) {
                    ...
                    if (repeatStatus == "once") {
                        ...
                        Log.d(
                            "debugging",
                            "in alarm receiver, changed the time of repeated alarm after it fired : $fetchedAlarm"
                        )
                    }
                }
            } else if ((repeatStatus != "once" && LocalDate.now() > LocalDate.parse(
                    repeatEnd,
                    DateTimeFormatter.ofPattern("dd/MM/yyyy", Locale.ENGLISH)
                ))
            ) {
