...
fun createWidget(appWidgetIds: IntArray, context: Context) {
    val sharedPref: SharedPreferences = context.getSharedPreferences(
        "ApplicationListener", Context.MODE_PRIVATE
    )
    val timeTypeHash: HashMap<String, String> = HashMap<String, String>()
    timeTypeHash["Day"] = "day"
    timeTypeHash["Week"] = "week"
    timeTypeHash["Month"] = "month"
    timeTypeHash["Year"] = "year"
    timeTypeHash["Long Term"] = "longTerm"
                ...
                for (todo in todos!!) {
                    if (!todo.finished!!) {
                        val view = RemoteViews(context.packageName, R.layout.each_todo).apply {
                            ...
                            Log.d("debugging", "task name in widget is ${todo.taskName}")

                            val extras = Bundle().apply {
                                putString("todoId", todo.id)
                            }
                            val editIntent = Intent(context, WidgetProvider::class.java).apply {
                                putExtras(extras);
                                action = "editTodo"
                            }
                            val checkIntent = Intent(context, WidgetProvider::class.java).apply {
                                putExtras(extras);
                                action = "checkTodo"
                            }
